...
\begin{mdframed}[style=longueur,frametitle={$\mathcal{P}$érimètre
    ...
    $\PP{polygone}=\text{somme des côtés}$\mynext
...
\begin{mdframed}[style=surface,frametitle={$\mathcal{A}$ire (\normalfont
    ...
    $\AA{parallélogramme}=\text{base}\times\text{hauteur}$\mynext
    $\AA{rectangle}=\text{longueur}\times\text{largeur}$\mynext
    $\AA{triangle}=\text{base}\times\text{hauteur}\div2$\mynext
